...
export default function ExploreScreen() {
  ...
  const ListHeader = (
    <View style={styles.headerPad}>
      <View style={styles.headerRow}>
        ...
        >
          <Ionicons
            ...
            size={16}
            color={showFilters ? MangAfrik.primary : MangAfrik.textSecondary}
          />
      ...
      <View style={styles.searchWrap}>
        <Ionicons name="search" size={16} color={MangAfrik.textMuted} style={styles.searchIcon} />
        <TextInput
          style={styles.searchInput}
          placeholder="Rechercher un manga, un auteur…"
          placeholderTextColor={MangAfrik.textMuted}
          value={query}
          onChangeText={setQuery}
        />
        {query.length > 0 && (
          <Pressable onPress={() => setQuery('')} style={styles.searchClear} hitSlop={8}>
            <Ionicons name="close-circle" size={20} color={MangAfrik.textMuted} />
          </Pressable>
        )}
      </View>
        ...
        <View style={[styles.activeFilters, { marginTop: 14 }]}>
          {selectedGenres.map((g) => (
            <Pressable key={g} onPress={() => toggleGenre(g)} style={styles.activeChip}>
              ...
              <Ionicons name="close" size={14} color={MangAfrik.primary} />
            </Pressable>
          ))}
        </View>
      )}
  ...
  const ListEmpty = (
    <View style={styles.empty}>
      <Ionicons name="book-outline" size={56} color={MangAfrik.textMuted} />
